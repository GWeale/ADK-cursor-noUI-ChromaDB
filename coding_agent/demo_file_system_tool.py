...
# Create a mock ToolContext for demo purposes
class MockState:
    def __init__(self):
...
# Create a minimal ToolContext-like object for demo
...
def demo_file_operations():
    ...
    # Create demo tool context
    # Note: In actual ADK usage, this would be provided by the framework
    tool_context = DemoToolContext()  # type: ignore
    
    # Read existing file
    print("1. Reading existing test file:")
    result = read_file("coding_agent/tests/test_file.txt", tool_context)  # type: ignore
    ...
    result = write_file("coding_agent/tests/demo_output.txt", demo_content, tool_context)  # type: ignore
    print(f"Result: {result}\n")
    
    # Read the newly created file
    print("3. Reading the newly created file:")
    result = read_file("coding_agent/tests/demo_output.txt", tool_context)  # type: ignore
    print(f"Result: {result}\n")
    
    # Read files from workspace root
    print("4. Reading example_code.py from workspace root:")
    result = read_file("example_code.py", tool_context)  # type: ignore
    print(f"Result: {result}\n")
    
    # try to access file outside project (should fail)
    print("5. Attempting to read file outside project (security test):")
    result = read_file("../../../etc/passwd", tool_context)  # type: ignore
    print(f"Result: {result}\n")
    
    # try to write file outside project (should fail)
    print("6. Attempting to write file outside project (security test):")
    result = write_file("../malicious_file.txt", "This should not be created", tool_context)  # type: ignore
    print(f"Result: {result}\n")
